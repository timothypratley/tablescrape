...
(defn series
  ...
  [s]
  (reverse (rest (take m (map second s)))))
(series sap)
...
(let [date (reverse (map (comp to-long first) sap))
...
(defn quantize-gains
  ...
  (let [quantiles (quantile ratios :probs bins)
        quantize (fn [ratio]
                   (count (take-while #(>= ratio %) quantiles)))
        quantized (map quantize ratios)
        gains (map / (drop period prices) prices)
...
(defn plot-quantized-gains
  ...
  (let [qpe #(quantize-gains prices ratios %)
        ...
        quantiles (range 1 (+ 2 (count bins)))
